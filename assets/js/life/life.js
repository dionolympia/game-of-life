...
// pow2():
// This function is used to round user input for the "Generation
// Step" argument to a power of 2.
...
// save_rewind.state():
// This function is called when the "Run", "Stop", or "Step"
// buttons is pressed to save the initial state of the canvas
// for the "Rewind" button to restore.
...
// This function restores the canvas to the initial state saved
// by the save_rewind_state function.
...
// set_bit():
// This function is used to draw pixels on the canvas where the
// user's mouse pointer clicks
...
// get_bit():
// This function tells the set_bit function whether or not the
// pixel it is setting is alive or dead, so that the set_bit
// function properly changes alive cells to dead cells when the
// mouse is clicked and vice versa.
...
// get_root_bounds():
// This function gets the boundaries of the current pattern on
// the canvas so that the "Fit Pattern" button can adjust to the
// correct zoom level.
...
// empty_tree():
// This function empties the cache that is used to store the canvas
// that is restored when the "Back" button is pressed.  Unlike the
// uncache() function, this does not preserve the tree.
...
// expand_universe():
// This function makes the "universe" hashmap grow as the nodes
// drawn on the canvas grow.
...
// unchache():
// This functions preserves the tree, but remove all cached
// generations forward.
...
// in_hashmap():
// This function returns false if a node is in the hashmap.
...
// hashmap_insert():
// This function inserts a node into the hashmap.
...
// create_tree():
// This fuction creates or searches for a tree node given its 
// children.
...
// next_generation():
// Handles all of the transformation of each node for each generation.
...
// garbage_collect():
// This functinos reduces the hashmap size of the universe to what
// is needed for the current pattern on the canvas.
...
// calc_hash():
// This function is the hash function used for the hashmap.
...
// clear_pattern():
// This function clears the pattern drawn on the canvas.
...
// get_bounds():
// This function sets the initial bounds of a pattern when it is
// created with the setup_field() function.
...
// get_level_from_bounds():
// This function, given a point { x, y } or a bounds object
// { left, top, bottom, right }, returns the quadtree level that 
// is required to contain this point.
...
// field2tree():
// This function converts a set field to nodes contained within
// a tree.
...
// make_center():
// This functions moves a field so that (0,0) is in the middle.
...
// move_field():
// This function moves a field, given the offsets for x & y.
...
// setup_field();
// This function sets up a field, given its position and bounds.
...
// partition():
// This functions separates the values from start to end by
// the bitmask in offset in the array test_field, returning 
// the middle (Similar to quicksort).
...
// setup_field_recurse():
// This function splits the field into 4 parts (nw, new, sw, se).
...
// level2_setup();
// This function sets up the tree for level 2, given the level 2 
// field.
...
// node_level2_next();
// This handles (the next generation) of the nodes diagonal to 
// each node on the hashmap.
